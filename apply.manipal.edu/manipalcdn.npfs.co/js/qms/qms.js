...
function getQMSData() {
    ...
    $.ajax({
        ...
        data: {
            qms_new_design_applicable: $("#qms_new_design_applicable").val()
        },
        type: 'post',
        dataType: 'html',
        headers: {
...
function LoadQMSDataNew(type, hide_modal, sortField, sortOrder) {
    ...
    data.push({
        name: "per_page_record",
        value: per_page_record
    });
...
function LoadMoreMyTickets(ListingType) {
    ...
    var data = {
        MyTicketStartPage: MyTicketStartPage,
...
function ShowTicketDetails(TicketNumber, AppendSection) {
    if (TicketNumber) {
        ...
        $.ajax({
            ...
            data: {
                TicketNumber: TicketNumber,
                counsellorEditApplicaton: jsVars.counsellorEditApplicaton
            },
            headers: {
                'X-CSRF-TOKEN': jsVars.csrfToken
            },
    ...
    $.ajax({
        ...
        success: function(json) {
            if (json['error'] == "session") {
                ...
                $('#assigned_to').SumoSelect({
                    placeholder: 'Assigned To',
                    search: true,
                    searchText: 'Assigned To',
...
function changeChildCatList(RadioCategoryId, SelectedChildCategoryId, assign_type) {
    ...
    $.ajax({
        ...
        data: {
            id: RadioCategoryId,
            select: SelectedChildCategoryId,
            CollegeId: CollegeId,
            action: 'get',
